...
from devkit.sparse_ops import SparseLinear


class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
                 N=4, M=4, search=False):
        super().__init__() 
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.proj = SparseLinear(dim, dim, N = N, M = M, search = search)
        self.proj_drop = nn.Dropout(proj_drop)
        self.qkv = SparseLinear(dim, dim * 3, bias=qkv_bias, N = N, M = M, search = search)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 fused_window_process=False, N = 4, M = 4, search = False,):
        super().__init__()
        self.dim = dim
...
class SparseSwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, fused_window_process=False, N = 4, M = 4, search = False,
                 **kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               fused_window_process=fused_window_process, N = N, M = M,
                               search = search)
...
def sparse_swin_small_patch16_224(pretrained=False, N = 2, M = 4, search = False, **kwargs):
    model = SparseSwinTransformer(
        img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        embed_dim=96, depths=[2, 2, 18, 2], num_heads=[3, 6, 12, 24],
        window_size=7, N = N, M = M, search = search
    )
    if pretrained:
...
def sparse_swin_base_patch16_224(pretrained=False, N = 2, M = 4, search = False, **kwargs):
    model = SparseSwinTransformer(
        img_size=224, patch_size=4, in_chans=3, num_classes=1000,
